...
public final class Pirate extends Combatant {
    ...
    boolean findLoot() {
        ...
        if (loot.size() > 0) {
            ...
            adjustedValue("score", item.getWorth());
    ...
    private boolean visitNextTown() {
        ...
        if (townIndex >= (towns.size() - 1)) {
            ...
            adjustedValue("score", 500);
            adjustedValue("level", 1);
            setValue("townIndex", 0);
        } else {
            System.out.println("Sailing to next town! Bonus: 50 points!");
            adjustedValue("townIndex", 1);
            adjustedValue("score", 50);
        }
        return visitTown();
    }
}
