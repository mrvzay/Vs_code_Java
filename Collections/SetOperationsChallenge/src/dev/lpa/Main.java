...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        Set<Task> overlap = getUnion(List.of(
                getInterset(annsTasks, bobsTasks),
                getInterset(carolTasks, bobsTasks),
                getInterset(annsTasks, carolTasks)
        ));
        sortAndPrint("Assigned to Multiples", overlap, sortByPriority);

        List<Task> overlapping = new ArrayList<>();
        for (Set<Task> set : sets) {
            Set<Task> dupes = getInterset(set, overlap);
    ...
    private static Set<Task> getInterset(Set<Task> a, Set<Task> b) {
